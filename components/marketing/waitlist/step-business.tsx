import { RefObject } from "react";
import { motion, AnimatePresence } from "framer-motion";

interface Option {
  emoji: string;
  label: string;
}

interface StepBusinessProps {
  businessType: string;
  customBusinessType: string;
  options: Option[];
  customRef: RefObject<HTMLInputElement | null>;
  onSelect: (val: string) => void;
  onCustomChange: (val: string) => void;
  onAdvance: () => void;
  onGoBack: () => void;
  isValid: boolean;
}

export function StepBusiness({
  businessType,
  customBusinessType,
  options,
  customRef,
  onSelect,
  onCustomChange,
  onAdvance,
  onGoBack,
  isValid
}: StepBusinessProps) {
  return (
    <div>
      <button onClick={onGoBack} className="text-xs text-[#6f6b63] mb-3 hover:text-[#171717] transition flex items-center gap-1">← Back</button>
      <h3 className="font-serif text-lg font-bold text-[#171717] mb-3">What type of business do you run?</h3>
      
      <div className="grid grid-cols-2 gap-2 max-h-[160px] overflow-y-auto mb-4 pr-1 custom-scrollbar">
        {options.map((opt) => (
          <button
            key={opt.label}
            onClick={() => onSelect(opt.label)}
            className={`p-3 rounded-xl text-left text-xs font-medium border flex items-center gap-2 transition ${businessType === opt.label ? "border-[#943333] bg-[#943333]/5 text-[#943333]" : "border-[#e7e2d8] text-[#171717] hover:bg-[#fcfaf7]"}`}
          >
            <span>{opt.emoji}</span>
            <span className="truncate">{opt.label}</span>
          </button>
        ))}
      </div>

      <AnimatePresence>
        {businessType === "Other" && (
          <motion.div 
            initial={{ opacity: 0, height: 0 }}
            animate={{ opacity: 1, height: "auto" }}
            exit={{ opacity: 0, height: 0 }}
            className="overflow-hidden mb-4"
          >
            <input
              ref={customRef}
              type="text"
              value={customBusinessType}
              onChange={(e) => onCustomChange(e.target.value)}
              placeholder="Please specify your business type..."
              className="w-full px-4 py-2.5 rounded-xl border border-[#e7e2d8] bg-[#fcfaf7] focus:border-[#943333] focus:ring-1 focus:ring-[#943333] outline-none transition text-xs text-[#171717]"
            />
          </motion.div>
        )}
      </AnimatePresence>

      <button 
        disabled={!isValid} 
        onClick={onAdvance} 
        className="w-full bg-[#943333] text-white font-medium py-3 rounded-full hover:bg-[#b83f20] disabled:bg-[#b83f3f] disabled:text-[#9c968e] transition text-sm shadow-sm"
      >
        Continue
      </button>
    </div>
  );
}