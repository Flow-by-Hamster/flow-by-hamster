...
export function FeaturedPostCard({ post }: FeaturedPostCardProps) {
  return (
    <div className="mb-16">
      <Link 
        href={`/blog/${post.slug}`} 
        className="group grid grid-cols-1 md:grid-cols-12 gap-8 items-center bg-white border border-[#e7e2d8] p-6 md:p-8 rounded-3xl hover:border-[#943333]/30 hover:shadow-[0_12px_40px_rgba(23,23,23,0.02)] transition-all duration-300"
      >
        <div className="md:col-span-7 space-y-4">
          <div className="flex items-center gap-2.5 text-[11px] font-bold text-[#8c877d]">
            <span className="text-[#943333] uppercase tracking-wider bg-[#fcfaf7] border border-[#e7e2d8] px-2.5 py-0.5 rounded-full">
              Featured Article
            </span>
            <span>·</span>
            <span>{post.readTime}</span>
          </div>
          <h2 className="text-2xl md:text-3xl font-bold tracking-tight text-[#171717] group-hover:text-[#943333] font-serif transition-colors duration-200">
            {post.title}
          </h2>
        ...
        <div className="md:col-span-5 h-48 md:h-64 w-full bg-[#f4efe8] rounded-2xl border border-[#e7e2d8] relative overflow-hidden">
          ...
          <div className="absolute inset-0 bg-[#943333]/10 mix-blend-multiply z-10 pointer-events-none transition-colors duration-300 group-hover:bg-[#943333]/5" />
